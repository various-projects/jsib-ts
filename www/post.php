...
$boardId  = $threadPath[0];
$threadIndex = $threadPath[2];

define('THUMBNAIL_IMAGE_MAX_WIDTH', 150);
define('THUMBNAIL_IMAGE_MAX_HEIGHT', 150);
require './settings.php';

$threadPath = "boards/" . $threadPath;
    ...
    if ($imgError !== "") {
        ...
        if (move_uploaded_file($_FILES["file"]["tmp_name"], $targetFilePath)) {
            if (generateThumb($targetFilePath, "$threadPath/thumb/$targetFileName"))
                ...
                unlink($targetFilePath); //delete the file if thumbnail generation failed
        } else {
...
$sizeIndexFileName = "$threadPath/postIndex.txt";

$fileStart = microtime(true);
$delimiter = ",\r\n";
$serialized_message = $delimiter . json_encode($messageData, JSON_UNESCAPED_UNICODE);
file_put_contents($datafileName, $serialized_message, FILE_APPEND);
file_put_contents($lastPostDataFileName, $serialized_message);
file_put_contents($sizeIndexFileName, strlen($serialized_message) . "\r\n", FILE_APPEND);
...
if (preg_match("/Windows/", php_uname("s")))
    ...
    $lsOutput = shell_exec("dir /b /o-d boards\\$boardId"); // 'b' for 'bare' output, 'o' for order (sort) by 'd'ate '-' for reverse order (newest first)
else
    //Linux, UNIX, MacOS X(?)
...
file_put_contents("boards/$boardId/threads.json", $threads); //TODO: check for locking?
